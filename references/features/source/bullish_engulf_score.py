"""
Feature: bullish_engulf_score
Contract version: 1.0.0
Category: candlestick_pattern
Normalization mode: sparse_positive

Purpose:
    Scores a completed bullish engulfing pattern by its body size relative to ATR.

Formula:
    I[bullish_engulfing(t-2,t-1)] * |close[t-1]-open[t-1]| / (ATR_14_shift1[t] + EPS)
...
Output:
    ...
    Interpretation: Zero means no bullish engulfing event; larger values indicate a stronger ATR-scaled pattern.
...
def compute(g: pd.DataFrame) -> pd.Series:
    ...
    is_engulf = (
        (prev_close < prev_open)  # t-2 bearish
        & (curr_close > curr_open)  # t-1 bullish
